...
def test_list_json_output(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert set(payload["engines"]) == {"ollama", "lmstudio", "omlx", "turboquant"}
...
def test_status_single_engine_json(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    assert payload == {
        "engines": [
            {"engine": "ollama", "state": "stopped", "port": 11434,
...
def test_install_invokes_lifecycle_install_with_firewall() -> None:
    fake_result = {"engine": "ollama", "binary": "/x", "plist": "/y",
                   "anchor": "/z", "health_ok": True, "dry_run": False}
    with patch("ais_cli.commands.lifecycle.install", return_value=fake_result) as m, \
...
def test_install_dry_run_returns_zero() -> None:
    fake_result = {"engine": "ollama", "binary": "/x", "plist": "/y",
                   "anchor": None, "health_ok": None, "dry_run": True}
    with patch("ais_cli.commands.lifecycle.install", return_value=fake_result), \
         patch("ais_cli.commands.memory.OperationsLock") as mock_lock:
        mock_lock.return_value.__enter__.return_value = mock_lock.return_value
...
def test_install_unhealthy_returns_nonzero() -> None:
    fake_result = {"engine": "ollama", "binary": "/x", "plist": "/y",
                   "anchor": None, "health_ok": False, "dry_run": False}
    with patch("ais_cli.commands.lifecycle.install", return_value=fake_result), \
         patch("ais_cli.commands.memory.OperationsLock") as mock_lock:
        mock_lock.return_value.__enter__.return_value = mock_lock.return_value
...
def test_unload_with_model_calls_driver_unload() -> None:
    ...
    with patch.dict(
        "ais_cli.commands.DRIVER_FACTORIES", {"ollama": factory}, clear=False
    ), patch("ais_cli.commands.memory.OperationsLock") as mock_lock:
        mock_lock.return_value.__enter__.return_value = mock_lock.return_value
...
def test_purge_dry_run_emits_report() -> None:
    ...
    with patch("ais_cli.commands.memory.purge_memory", return_value=fake_report) as m, \
         patch("ais_cli.commands.memory.OperationsLock") as mock_lock:
        mock_lock.return_value.__enter__.return_value = mock_lock.return_value
...
def test_repair_runs_without_lock() -> None:
    ...
    with patch("ais_cli.commands.memory.repair") as m_repair, \
...
def test_bootstrap_install_invokes_install_sudoers(
    ...
    with patch("ais_cli.commands.sudoers.validate_content"), \
         patch(
